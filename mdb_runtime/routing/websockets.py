...
This module provides OPTIONAL WebSocket support for MDB_RUNTIME apps via manifest.json configuration.
...
- Automatic authentication: Integrates with mdb_runtime auth system
...
class WebSocketConnectionManager:
    ...
    async def connect(
        ...
        logger.info(
            ...
            f"(user: {user_email or 'anonymous'}). Total connections: {len(self.active_connections)}"
        )
        return connection
    ...
    def disconnect(self, websocket: Any) -> None:
        ...
        async def _disconnect():
            ...
            logger.info(f"WebSocket disconnected for app '{self.app_slug}'. Remaining connections: {len(self.active_connections)}")
    ...
    try:
        ...
        if hasattr(websocket, 'query_params'):
            if websocket.query_params:
                ...
                logger.info(f"WebSocket query_params for app '{app_slug}': {dict(websocket.query_params)}")
                if query_token:
                    token = query_token
                    logger.info(f"WebSocket token found in query params for app '{app_slug}' (length: {len(query_token)})")
            else:
        ...
        logger.info(f"WebSocket auth check for app '{app_slug}': query_token={bool(query_token)}, cookie_token={bool(cookie_token)}, final_token={bool(token)}, require_auth={require_auth}")
        
        if not token:
            logger.warning(f"No token found for WebSocket connection to app '{app_slug}' (require_auth={require_auth})")
            if require_auth:
        ...
        secret_key = os.environ.get("FLASK_SECRET_KEY") or os.environ.get("SECRET_KEY") or "a_very_bad_dev_secret_key_12345"
...
def register_message_handler(
    ...
    Example:
        ...
        ```
    """
    # Note: This is called synchronously during app startup, so no async lock needed
    # The lock is only used in async contexts (get_message_handler)
...
def get_message_handler(app_slug: str, endpoint_name: str) -> Optional[Callable[[Any, Dict[str, Any]], Awaitable[None]]]:
    """
    ...
    """
    # Note: This is called synchronously during route creation.
    # Registration happens at startup (single-threaded), and reads are safe.
    # The async lock is only needed if we add async registration later.
...
def create_websocket_endpoint(
    ...
    Args:
        ...
        handler: Optional async function to handle incoming messages (deprecated, use register_message_handler instead).
                 Signature: async def handler(websocket, message: Dict[str, Any]) -> None
    ...
    if not WEBSOCKETS_AVAILABLE:
        raise ImportError(
            ...
            "Install with: pip install 'fastapi[standard]' or ensure WebSocket support is available."
        )
    ...
    async def websocket_endpoint(websocket: websocket_type):
        ...
        import sys
        print(f"🔌 [WEBSOCKET HANDLER CALLED] App: '{app_slug}', Path: {path}", file=sys.stderr, flush=True)
        ...
        try:
            ...
            print(f"🔌 [WEBSOCKET HANDLER CALLED] App: '{app_slug}', Path: {path}, Query: {query_str}")
            logger.info(f"🔌 WebSocket connection attempt for app '{app_slug}' (require_auth={require_auth}, query_params={query_str})")
            ...
            try:
                ...
                logger.error(f"❌ Failed to accept WebSocket for app '{app_slug}': {accept_error}", exc_info=True)
                raise
            ...
            try:
                ...
                logger.error(f"Unexpected error during WebSocket authentication for app '{app_slug}': {auth_error}", exc_info=True)
                try:
                    ...
                    if message.get("type") == "websocket.disconnect":
                        ...
                        if "text" in message:
                            try:
                                ...
                                # Get handler dynamically (may be registered after route creation)
                                # Prefer passed handler, then registered handler
                                current_handler = handler if handler else get_message_handler(app_slug, endpoint_name)
                                
                                # Route to handler if available
                                # Handlers can process client messages and respond
                                if current_handler:
                                    try:
                                        await current_handler(websocket, data)
                                    except Exception as handler_error:
                                        logger.error(
                                            f"Error in WebSocket message handler for app '{app_slug}': {handler_error}",
                                            exc_info=True
                                        )
                                    ...
                                    logger.debug(
                                        f"Received message on WebSocket for app '{app_slug}' but no handler registered. "
                                        f"Use register_message_handler() to handle client messages."
                                    )
                                    
                            except json.JSONDecodeError:
                                logger.debug(f"Received non-JSON message from WebSocket client")
...
async def broadcast_to_app(app_slug: str, message: Dict[str, Any], user_id: Optional[str] = None) -> int:
    """
