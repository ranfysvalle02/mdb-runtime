# WebSocket Support in MDB_RUNTIME

MDB_RUNTIME provides built-in WebSocket support with app-level isolation and automatic route registration via `manifest.json`.
...
```python
from mdb_runtime.routing.websockets import register_message_handler, broadcast_to_app
...
- Integrates with mdb_runtime's JWT authentication system
...
```python
from mdb_runtime.routing.websockets import register_message_handler, get_websocket_manager
...
MDB_RUNTIME WebSockets support **both directions**:
