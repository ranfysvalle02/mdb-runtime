...
async def seed_initial_data(
    ...
    for collection_name, documents in initial_data.items():
        try:
            ...
            if count > 0:
                logger.info(
                    f"Collection '{collection_name}' is not empty ({count} documents) for {app_slug}, "
            ...
            if prepared_docs:
                ...
                logger.info(
                    f"✅ Seeded {inserted_count} document(s) into collection '{collection_name}' for {app_slug}"
                )
            else:
                results[collection_name] = 0
    ...
    try:
        ...
        logger.warning(f"Failed to update seeding metadata for {app_slug}: {e}")
