...
# Import SECRET_KEY from mdb_engine.auth.dependencies
...
async def sync_sub_auth_user_to_casbin(
    user: Dict[str, Any],
    authz_provider,
    ...
    """
    Sync sub_auth user to Casbin by assigning a role.
    
    This function automatically assigns a Casbin role to a sub_auth user,
    linking the app user ID to the role in Casbin's grouping rules.
    
    Args:
        user: Sub-auth user dict (must have _id or app_user_id)
    ...
    try:
        ...
        if not hasattr(authz_provider, '_enforcer'):
            logger.debug("sync_sub_auth_user_to_casbin: Provider is not CasbinAdapter, skipping")
            return False
        ...
        if not user_id:
            logger.warning("sync_sub_auth_user_to_casbin: User has no _id or app_user_id")
            return False
        ...
        if role in existing_roles:
            logger.debug(f"sync_sub_auth_user_to_casbin: User {subject} already has role {role}")
            return True
        ...
        if added:
            ...
            logger.info(
                f"sync_sub_auth_user_to_casbin: Assigned role '{role}' to user '{subject}'"
                + (f" in app '{app_slug}'" if app_slug else "")
            )
            return True
        else:
            logger.warning(f"sync_sub_auth_user_to_casbin: Failed to assign role '{role}' to user '{subject}'")
            return False
            
    except Exception as e:
        logger.error(f"sync_sub_auth_user_to_casbin: Error syncing user to Casbin: {e}", exc_info=True)
        return False


def get_sub_auth_user_role(user: Dict[str, Any], config: Optional[Dict[str, Any]] = None) -> str:
    """
    Determine Casbin role for sub_auth user.
