#!/usr/bin/env python3
"""
Oblivio Auth Hub - SSO Authentication Hub
==========================================
...
async def register(
    ...
    try:
        ...
        user = await pool.create_user(
            ...
            password=password,
            app_roles={
                APP_SLUG: ["viewer"],  # Default role for auth hub
                "oblivio-sso-app-1": ["viewer"],  # Grant access to SSO apps
                "oblivio-sso-app-2": ["viewer"],
                "oblivio-sso-app-3": ["viewer"],
            },
        )
...
async def dashboard(request: Request):
    ...
    return templates.TemplateResponse(
        "dashboard.html",
        {
            ...
            "all_users": all_users,
            "sso_apps": ["oblivio-sso-app-1", "oblivio-sso-app-2", "oblivio-sso-app-3"],
        },
    )
...
async def grant_access(
    ...
    """Grant viewer access to an SSO app (admin only)."""
    user = get_current_user(request)
    if not user or not is_admin(user):
        raise HTTPException(403, "Admin access required")
    ...
    # Add viewer role if not present
    if "viewer" not in current_roles:
        current_roles.append("viewer")
    ...
    return {
        ...
        "email": email,
        "app_slug": app_slug,
        "message": f"Granted viewer access to {app_slug}",
    }
...
if __name__ == "__main__":
    ...
    logger.info("Starting Oblivio Auth Hub on 0.0.0.0:8000")
    try:
        uvicorn.run(
